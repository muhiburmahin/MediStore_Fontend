...
export default async function DashboardPage() {
        ...
        return (
            <div className="h-96 flex items-center justify-center p-10">
                <div className="bg-red-50 dark:bg-red-900/20 p-6 rounded-2xl border border-red-100 dark:border-red-800 text-center">
                    <h1 className="text-red-600 dark:text-red-400 font-black uppercase italic tracking-widest">
                        Error Occurred
                    </h1>
                    <p className="text-red-400 dark:text-red-500/70 text-sm mt-2">
    ...
    return (
        <div className="p-4 md:p-6 lg:p-10 space-y-10 bg-slate-50 dark:bg-[#020617] min-h-screen transition-colors duration-500">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] mt-2 ml-1">
                        MediStore Pro Suite • Real-time Sync Active
                    </p>
                </div>
                <div className="flex items-center w-fit gap-3 bg-white dark:bg-slate-900 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {cardData.map((item) => (
                    <div key={item.name} className="group bg-white dark:bg-slate-900 p-6 rounded-[24px] md:rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300">
                        ...
                        <div className="mt-6">
                            ...
                            <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight italic">{item.value.toLocaleString()}</h3>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid gap-6 md:gap-8 lg:grid-cols-2 xl:grid-cols-3">
                {/* Inventory Chart */}
                <div className="bg-white dark:bg-slate-900 p-4 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-1.5 h-6 bg-blue-600 dark:bg-blue-500 rounded-full"></div>
                        <h4 className="font-black text-slate-800 dark:text-slate-200 uppercase tracking-tighter italic text-sm md:text-base">Inventory Split</h4>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h4 className="font-black text-slate-800 dark:text-slate-200 uppercase tracking-tighter italic flex items-center gap-2 text-sm md:text-base">
                ...
                {/* Revenue Stream Card */}
                <div className="bg-white dark:bg-slate-900 p-4 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm lg:col-span-2 xl:col-span-1">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-1.5 h-6 bg-indigo-600 dark:bg-indigo-500 rounded-full"></div>
                        <h4 className="font-black text-slate-800 dark:text-slate-200 uppercase tracking-tighter italic flex items-center gap-2 text-sm md:text-base">
                            Revenue Stream <DollarSign size={16} className="text-indigo-600" />
                        </h4>
                    </div>

                    <div className="flex flex-col items-center justify-center h-[300px] md:h-[350px] bg-slate-50/50 dark:bg-slate-800/20 rounded-3xl relative overflow-hidden">
