...
export const orderService = {
    createOrder: async (data: CreateOrder) => {
        console.log("Requesting to:", `${API_URL}/orders`);
        try {
    ...
    getSellerOrders: async () => {
        try {
            ...
            if (!res.ok) {
                const errBody = await res.json().catch(() => null);
                return { data: null, error: { message: errBody?.message ?? "Failed to get seller orders" } };
            }
    ...
    updateOrderById: async (id: string, orderData: { status: OrderStatus }) => {
        try {
            const cookieStore = await cookies();
            const res = await fetch(`${API_URL}/orders/update-status/${id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Cookie: cookieStore.toString(),
    ...
    getMyOrders: async () => {
        try {
            ...
            if (!res.ok) {
                return { data: [], error: result.message || "Failed to fetch orders" };
            }
            const orders = result.data || result || [];
            return { data: Array.isArray(orders) ? orders : [], error: null };
        } catch (error) {
